const fs = require('fs');
let lines = fs.readFileSync('src/components/MemberCard.tsx', 'utf8').split('\n');

const startIdx = lines.findIndex(l => l.includes('{!registeredUser ? ('));
const endIdx = lines.findIndex((l, i) => i > startIdx && l.includes(') : (activeSection === "profil" ? ('));

if (startIdx !== -1 && endIdx !== -1) {
  const newLines = `      {!registeredUser ? (
  ...
  fs.writeFileSync('src/components/MemberCard.tsx', lines.join('\n'));
}
